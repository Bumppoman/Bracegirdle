Rails.application.reloader.to_prepare do
  Dir.glob(Rails.root.to_s + '/app/consumers/**/*.rb').each do |file|
    File.basename(file, '.rb').camelize.constantize
  end
  
  Dir.glob(Rails.root.to_s + '/app/events/**').each do |dir|
    if File.directory?(dir)
      Dir.glob(Rails.root.to_s + "/app/events/#{dir}/**/*.rb").each do |file|
        File.basename(file, '.rb').camelize.constantize
      end
    else
      File.basename(dir, '.rb').camelize.constantize
    end
  end
end
